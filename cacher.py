...
def _translate_plex_path(plex_path: str) -> str:
    ...
    log.warning(
        ...
        "path will not be translated",
        plex_path, config.PLEX_PATH_PREFIX,
    )
    return plex_path
...
def get_cache_status(user_share_path: str) -> dict:
    ...
    if rel is None:
        log.warning(
            "Path %r does not start with USER_SHARE_BASE %r — returning not_found",
            user_share_path, config.USER_SHARE_BASE,
        )
    ...
    log.warning("File not found on cache (%s) or any array disk (pattern: %s)", cache_path, pattern)
    return {"status": "not_found"}
